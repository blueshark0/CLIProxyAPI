...
func TestStageHomeOverlayPreservesLocalAllowedDomains(t *testing.T) {
	baseCfg := &config.Config{AllowedDomains: []string{"api.example.com"}}
	baseCfg.Home.Enabled = true
	client, _ := newHomePluginTaskTestClient(t, nil, 0)
	service := &Service{cfg: baseCfg}

	remoteCfg := &config.Config{AllowedDomains: []string{"remote.example.com"}}
	...
	if work.config == nil {
		t.Fatal("expected staged home overlay config")
	}
	if len(work.config.AllowedDomains) != 1 || work.config.AllowedDomains[0] != "api.example.com" {
		t.Fatalf("expected home overlay to preserve local allowed domains, got %v", work.config.AllowedDomains)
	}
}
